namespace MsSqlToSqlLiteConverter
{
    ...
    public class SqlServerToSQLite
    {
        ...
        /// convert.</param>
        public static void ConvertSqlServerDatabaseToSqLiteFile(
            string sqlConnString, string sqlitePath, string insertscriptpath, string password, IList<string> excludeTables, SqlConversionHandler handler,
            SqlTableSelectionHandler selectionHandler,
            FailedViewDefinitionHandler viewFailureHandler,
            bool createTriggers)
        {
            Debug.WriteLine($"Starting {nameof(ConvertSqlServerDatabaseToSqLiteFile)}");
            // Delete the target file if it exists already.
            if (File.Exists(sqlitePath))
                File.Delete(sqlitePath);

            // Read the schema of the SQL Server database into a memory structure
            DatabaseSchema ds = ReadSqlServerSchema(sqlConnString, excludeTables, handler, selectionHandler);
            ...
            CopySqlServerRowsToSQLiteDB(sqlConnString, sqlitePath, insertscriptpath, ds.Tables, password, handler);
        ...
        /// <param name="insertscriptpath"></param>
        /// <param name="tables">The schema of the SQL Server database.</param>
        /// <param name="password">The password to use for encrypting the file</param>
        /// <param name="handler">A handler to handle progress notifications.</param>
        private static void CopySqlServerRowsToSQLiteDB(string sqlConnString, string sqlitePath, string insertscriptpath, List<TableSchema> tables, string password, SqlConversionHandler handler)
        {
            CheckCancelled();
            handler?.Invoke(false, true, 0, "Preparing to insert tables...");
            ...
            using (SqlConnection ssconn = new SqlConnection(sqlConnString))
            {
                ...
                using (SQLiteConnection sqconn = new SQLiteConnection(sqliteConnString))
                {
                    ...
                    commandText.AppendLine(" PRAGMA foreign_keys = OFF;");
                    for (int i = 0; i < tables.Count; i++)
                    {
                        SQLiteTransaction tx = sqconn.BeginTransaction();
                        try
                        {
                            string tableQuery = BuildSqlServerTableQuery(tables[i]);
                            SqlCommand query = new SqlCommand(tableQuery, ssconn);
                            using (SqlDataReader reader = query.ExecuteReader())
                            {
                                SQLiteCommand insert = BuildSQLiteInsert(tables[i]);
                                int counter = 0;
                                while (reader.Read())
                                {
                                    ...
                                    var cd = insert.CommandText;
                                    for (int j = 0; j < tables[i].Columns.Count; j++)
                                    {
                                        string pname = "@" + GetNormalizedName(tables[i].Columns[j].ColumnName, pnames);
                                        insert.Parameters[pname].Value = CastValueForColumn(reader[j], tables[i].Columns[j]);
                                        var value = ProcessValue(tables[i].Columns[j], insert.Parameters[pname].Value);
                                        var reg = new Regex($@"({pname}\b)");
                                        cd = reg.Replace(cd, value);
                                        pnames.Add(pname);
                                    }
                                    ...
                                    if (counter % 1000 == 0)
                                    {
                                        CheckCancelled();
                                        tx.Commit();
                                        handler?.Invoke(false, true, (int)(100.0 * i / tables.Count),
                                            "Added " + counter + " rows to table " + tables[i].TableName + " so far");
                                        tx = sqconn.BeginTransaction();
                                    }
                                    commandText.AppendLine(cd + ";");
                                } // while
                            } // using
                            ...
                            handler?.Invoke(false, true, (int)(100.0 * i / tables.Count), "Finished inserting rows for table " + tables[i].TableName);
                            _log.Debug("finished inserting all rows for table [" + tables[i].TableName + "]");
                        }
                        catch (Exception ex)
                        {
                    ...
                    commandText.AppendLine(" PRAGMA foreign_keys = ON;");

                    using (StreamWriter outputFile = new StreamWriter(insertscriptpath))
                    {
                        outputFile.Write(commandText.ToString());
                    }
                } // using
            } // using
        }

        private static string ProcessValue(ColumnSchema cs, object value)
        {
            var t = GetDbTypeOfColumn(cs);
            ...
            if (value != null)
            {
                ...
                switch (t)
                {
                    ...
                    case DbType.StringFixedLength:
                        val = string.Concat("'", val.Replace("'", "''"), "'");
                        break;
                    case DbType.Guid:
                        val = string.Concat("X'", val.Replace("'", "''"), "'");
                        break;
                    case DbType.Boolean:
                        val = ((bool)value) ? "1" : "0";
                        break;
        ...
        private static SQLiteCommand BuildSQLiteInsert(TableSchema ts)
        {
            ...
            {
                if (string.Compare(ts.Columns[i].ColumnName, "index", StringComparison.InvariantCultureIgnoreCase) == 0)
            ...
            FailedViewDefinitionHandler viewFailureHandler)
        {
            ...
            using (SQLiteConnection conn = new SQLiteConnection(sqliteConnString))
            {
                ...
                foreach (TableSchema dt in schema.Tables)
                {
                    ...
                    count++;
                    CheckCancelled();
                    handler?.Invoke(false, true, (int)(count * 50.0 / schema.Tables.Count), "Added table " + dt.TableName + " to the SQLite database");
                ...
                foreach (ViewSchema vs in schema.Views)
                {
                    ...
                    count++;
                    CheckCancelled();
                    handler?.Invoke(false, true, 50 + (int)(count * 50.0 / schema.Views.Count), "Added view " + vs.ViewName + " to the SQLite database");
        ...
        private static DatabaseSchema ReadSqlServerSchema(string connString, IList<string> excludeTables, SqlConversionHandler handler,
            SqlTableSelectionHandler selectionHandler)
        {
            ...
            using (SqlConnection conn = new SqlConnection(connString))
            {
                ...
                for (int i = 0; i < tableNames.Count; i++)
                {
                    ...
                    if (excludeTables.Contains(tschma + "." + tname))
                        continue;
                    ...
                    count++;
                    CheckCancelled();
                    handler?.Invoke(false, true, (int)(count * 50.0 / tableNames.Count), "Parsed table " + tname);
            ...
            using (SqlConnection conn = new SqlConnection(connString))
            {
                ...
                using (SqlDataReader reader = cmd.ExecuteReader())
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        count++;
                        CheckCancelled();
                        handler?.Invoke(false, true, 50 + (int)(count * 50.0 / views.Count), "Parsed view " + vs.ViewName);
